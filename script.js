...
const MUSIC_STORAGE_KEY = 'fighting-soundtrack-track-v2';
const SFX_STORAGE_KEY = 'personal-home-sfx';
const DISCORD_LINK = 'https://discord.com/invite/1230904398742294660';
const STEAM_LINK = 'https://steamcommunity.com/profiles/76561199145094521/';
...
const OPTIMIZED_SOUNDTRACKS = [
  ...
  },
  {
    ...
    tracks: [
      { title: 'Brimstone & Fire 1st', file: 'brimstone-fire-1st.mp3' }
    ]
  },
  {
    id: 'tekken-8',
...
SOUNDTRACKS = OPTIMIZED_SOUNDTRACKS.filter((game) => game.id === 'tekken-7');
...
function storeSfxPreference(isEnabled) {
  try {
    localStorage.setItem(SFX_STORAGE_KEY, isEnabled ? 'on' : 'off');
  } catch (error) {
    // SFX still works for this page when storage is unavailable.
  }
}
...
function playTone(frequency, duration = 0.08, options = {}) {
  ...
  const volume = options.volume ?? 0.028;
...
function playUiSound(kind) {
  ...
  if (kind === 'hover') {
    playTone(880, 0.035, { endFrequency: 1120, volume: 0.014, type: 'triangle' });
    return;
  }

  if (kind === 'select') {
    playTone(520, 0.055, { endFrequency: 780, volume: 0.022 });
    window.setTimeout(() => playTone(1040, 0.05, { volume: 0.018, type: 'triangle' }), 45);
    return;
  }

  if (kind === 'back') {
    playTone(360, 0.07, { endFrequency: 240, volume: 0.02, type: 'sawtooth' });
    return;
  }

  if (kind === 'error') {
    playTone(180, 0.1, { endFrequency: 90, volume: 0.025, type: 'sawtooth' });
    return;
  }

  if (kind === 'hadouken') {
    playTone(140, 0.12, { endFrequency: 520, volume: 0.032, type: 'sawtooth' });
    window.setTimeout(() => playTone(920, 0.12, { endFrequency: 460, volume: 0.024 }), 80);
    return;
  }

  playTone(640, 0.045, { endFrequency: 420, volume: 0.024 });
}

function updateSfxToggle(button) {
  if (!button) return;
  button.textContent = sfxEnabled ? 'SFX ON' : 'SFX OFF';
  button.setAttribute('aria-pressed', String(sfxEnabled));
  button.classList.toggle('is-muted', !sfxEnabled);
}
...
function setupInteractiveFeedback() {
  sfxEnabled = getSfxPreference();
  const sfxToggle = document.getElementById('sfx-toggle');
  updateSfxToggle(sfxToggle);

  document.getElementById('discord-card')?.setAttribute('data-clickable-card', DISCORD_LINK);
  document.getElementById('steam-card')?.setAttribute('data-clickable-card', STEAM_LINK);

  document.querySelectorAll('[data-clickable-card]').forEach((card) => {
    card.setAttribute('role', 'button');
    card.setAttribute('tabindex', '0');
    card.setAttribute('title', 'Abrir link');
  });

  sfxToggle?.addEventListener('click', () => {
    const nextState = !sfxEnabled;
    sfxEnabled = nextState;
    storeSfxPreference(nextState);
    updateSfxToggle(sfxToggle);
    if (nextState) {
      playUiSound('select');
    }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (interactive.matches('#sfx-toggle')) return;
    createClickBurst(event.clientX, event.clientY);

    if (interactive.matches('[data-theme-choice], [data-blurb-language], [data-soundtrack-game], [data-track-id]')) {
      playUiSound('select');
    } else if (interactive.matches('#music-prev, #music-pause')) {
      playUiSound('back');
    } else if (interactive.matches('#ryu-easter-egg')) {
      playUiSound('hadouken');
    } else {
      playUiSound('click');
    }
  });
  ...
  document.addEventListener('click', (event) => {
    const card = event.target.closest('[data-clickable-card]');
    if (!card || event.target.closest('a, button')) return;
    window.open(card.dataset.clickableCard, '_blank', 'noopener,noreferrer');
  });
...
function buildAllTracks(games) {
  return games.flatMap((game) => (game.tracks || []).map((track, index) => ({
    ...
    src: track.src || `assets/soundtracks/${game.folder}/audio/${track.file}`
  })));
}
  ...
  try {
    ...
    SOUNDTRACKS = manifest
      ...
      .map((game) => ({
        ...
        cover: game.cover,
        tracks: game.tracks
          .filter((track) => track && track.title && (track.file || track.src))
          .map((track) => ({
            title: track.title,
